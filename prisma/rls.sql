...
alter table public.bible_lesson_verses  enable row level security;
alter table public.bible_themes         enable row level security;
alter table public.bible_verse_themes   enable row level security;
alter table public.bible_review_states  enable row level security;

drop policy if exists "bible_lessons_owner_all" on public.bible_lessons;
create policy "bible_lessons_owner_all" on public.bible_lessons
  for all using (auth.uid() = user_id) with check (auth.uid() = user_id);

drop policy if exists "bible_verses_owner_all" on public.bible_verses;
create policy "bible_verses_owner_all" on public.bible_verses
  for all using (auth.uid() = user_id) with check (auth.uid() = user_id);

drop policy if exists "bible_themes_owner_all" on public.bible_themes;
create policy "bible_themes_owner_all" on public.bible_themes
  for all using (auth.uid() = user_id) with check (auth.uid() = user_id);

drop policy if exists "bible_review_states_owner_all" on public.bible_review_states;
create policy "bible_review_states_owner_all" on public.bible_review_states
  for all using (auth.uid() = user_id) with check (auth.uid() = user_id);

-- bible_lesson_verses: ownership inherited via parent lesson.
drop policy if exists "bible_lesson_verses_owner_all" on public.bible_lesson_verses;
create policy "bible_lesson_verses_owner_all" on public.bible_lesson_verses
  for all using (
    exists (select 1 from public.bible_lessons l where l.id = lesson_id and l.user_id = auth.uid())
  ) with check (
    exists (select 1 from public.bible_lessons l where l.id = lesson_id and l.user_id = auth.uid())
  );

-- bible_verse_themes: ownership inherited via parent theme.
drop policy if exists "bible_verse_themes_owner_all" on public.bible_verse_themes;
create policy "bible_verse_themes_owner_all" on public.bible_verse_themes
  for all using (
    exists (select 1 from public.bible_themes t where t.id = theme_id and t.user_id = auth.uid())
  ) with check (
    exists (select 1 from public.bible_themes t where t.id = theme_id and t.user_id = auth.uid())
  );
