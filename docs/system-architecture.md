...
> Snapshot kiến trúc hệ thống — phản ánh state codebase tại 2026-05-24.
...
- Dashboard reminders: fetched server-side on `/dashboard` load via `getDashboardReminders()` — returns gratitudePending, overdue tasks, today's tasks, and expired plans with incomplete tasks. Card shown above summary when any pending.
